...
마이페이지 「저장한 콘텐츠 소스」 메뉴는 화면이 없어 진입이 깨져 있다. 북마크한 해시태그 묶음은 `GET /v1/members/me/hashtag-bundles` 로 조회할 수 있고, 템플릿 북마크 API는 아직 없다.

**목적**

마이페이지에서 저장한 템플릿·해시태그를 탭으로 확인하고, 해시태그는 북마크 해제·복사를 할 수 있게 한다.
...
| FR-08 | 템플릿 탭: UI 셸 + `0건` 빈 목록. API 추후 | Must |
...
| 화면 이름 | 설명 | Figma |
|-----------|------|-------|
| SavedContentSourcesPage (템플릿) | 탭 + 0건 | node 2272-9952 |
