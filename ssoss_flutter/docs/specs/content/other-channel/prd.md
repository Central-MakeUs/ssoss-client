...
- [x] As a **가게 운영자**, I want to **다시 만들기로 동일 채널을 재생성**, so that **채널을 다시 고르지 않고 결과를 받을 수 있다**.
...
| FR-05 | 다시 만들기 모달 확인 시 동일 채널 input으로 생성 중 화면을 다시 띄운다. (채널 재선택 없음) | Must |
...
- 다른 채널 선택 페이지·네비·결과 모드·다시 만들기 재생성
