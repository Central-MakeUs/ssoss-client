...
| FR-12 | 탈퇴 시 사유(라디오 선택 + 기타 자유 입력)를 수집하고, 사유 저장 API 연동 후 서버에 전달한다. | Should |
...
> - FR-12: 사유 UI 수집은 완료. 사유 저장 API는 서버 스펙 확정 후 연동 (Follow-up, naver와 공유)
...
[홈/설정] → 회원 탈퇴 탭 → 확인 모달 → [탈퇴 사유 화면]
  → 사유 선택(기타 시 자유 입력) → 계정 탈퇴하기
  → 탈퇴 API 성공 (+ 예정: 사유 저장) → [탈퇴 완료 화면] 2초 → [로그인 화면]
```
...
| access 토큰 만료 | 전역 인터셉터가 refresh API 호출 후 원요청 재시도, 갱신 인증 실패 시 세션 만료 경고 토스트 후 로그인 화면 이동 |
