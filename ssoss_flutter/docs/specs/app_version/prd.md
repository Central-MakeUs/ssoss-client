...
앱 콜드 스타트(스플래시)에서 현재 OS·버전을 서버에 조회하고, 업데이트가 필요하면 스토어로만 유도하는 강제 업데이트 모달을 표시한다. 네트워크 오류 시에는 재시도 모달로 막고, 그 외 조회 실패 시에는 사용자를 막지 않는다.
...
- [x] As a **앱 사용자**, I want to **인터넷이 원활하지 않을 때 재시도할 수 있고**, so that **네트워크 복구 후 버전 확인을 이어갈 수 있다**.
- [x] As a **앱 사용자**, I want to **네트워크 외 조회 실패 시에는 앱을 계속 사용할 수 있고**, so that **일시적 서버 오류로 앱이 막히지 않는다**.
...
| FR-06 | 네트워크 오류 시 제목 `인터넷 연결이 원활하지 않습니다.`, 내용 `네트워크 연결 상태를 확인해주세요.`, 버튼 `재시도`만 표시한다 | Must |
| FR-07 | `재시도` 탭 시 모달을 닫고 버전 조회 API를 다시 호출한다. 실패하면 모달을 다시 표시하고, 성공하면 기존 분기(강제 업데이트/세션 복원)를 진행한다 | Must |
| FR-08 | 네트워크 외 조회 실패 시 강제 업데이트 없이 기존 세션 복원 흐름을 진행한다 | Must |
...
| 보안 | 비인증 API (`skipAuth`) |
| 오프라인 | 네트워크 오류 시 재시도 모달로 막고, 그 외 조회 실패 시 앱 이용을 허용한다 |
...
1. 앱 실행 → 스플래시
2. 버전 조회
3-a. `updateRequired == false` 또는 네트워크 외 실패 → 세션 복원 → 로그인/홈
3-b. `updateRequired == true` → 강제 모달 → `업데이트 하기` → 스토어 이동 (앱은 스플래시+모달 유지)
3-c. 네트워크 오류 → 재시도 모달 → `재시도` → 버전 조회 재호출
