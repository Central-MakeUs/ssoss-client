...
앱 실행 직후 스플래시에서 현재 OS·앱 버전으로 최소 지원 여부를 조회한다. 업데이트가 필요하면 닫을 수 없는 모달로 스토어 이동만 허용하고, 조회 실패 시에는 세션 복원을 그대로 진행한다.
...
[AppVersionCubit]
    ...
    └─ allowed (false 또는 조회 실패) → LoginBloc.sessionRestoreRequested
```
...
flowchart TD
  ...
  Store --> Modal
  Check -->|false 또는 실패| Restore["LoginBloc.sessionRestoreRequested"]
...
| 조회 실패 처리 | `allowed`로 취급 | PRD FR-06 — 사용자를 막지 않음 |
